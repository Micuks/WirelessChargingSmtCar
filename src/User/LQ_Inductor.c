...
void InductorNormal (void)
{
    ...
    if(MagneticField<40)
    {
       ...
       {
           LastAngle = Servo_Left_Max; // 记忆有效参数，记忆偏移方向
       }
       else
       {
           LastAngle = Servo_Right_Min; // 记忆有效参数，记忆偏移方向
       }
    }
    if ((Lleft2 < 18) && (Lright2 < 18))        // 两个值都小于20，电磁杆远离赛道，进入直角弯和急弯
    {
        if ((Lright2 < 10) && (Lleft2 < 10))    // 冲出赛道，检测不到电感值时舵机需要打死
        {
        ...
        else if (Lleft2 > Lright2)              // 左2电感值 大于 右2电感值
        {
            if (Lleft1 > Lright1)               // 并且 左1电感值 大于 右1电感值
                TempAngle = Servo_Left_Max;     // 小车右偏，左打死
        }
        else if (Lright2 > Lleft2)              // 左2电感值 小于 右2电感值
        {
            if (Lleft1 < Lright1)               // 并且 左1电感值 小于 右1电感值
                TempAngle = Servo_Right_Min;    // 小车左偏，右打死
        }

    }*/

    if(Lleft2 + Lright2<40)      // 两个值都小于20，电磁杆远离赛道，进入直角弯和急弯
    {
        if (Lleft1 + Lleft2 > Lright2 + Lright1)
        {
    ...
    {
        TempAngle = Servo_Center_Mid + (Lleft2 - Lright2) * 2; //  根据偏移量差值小幅度打角，防止直道摇摆
    }
    else                                        //  小车行走于赛道上弯道区域，一大一小，需要较大程度控制转向
    {
        TempAngle = Servo_Center_Mid + (Lleft2 - Lright2) * 7; //  一般弯道转向控制，数值越大，转向越早
    }
}
    ...
    if (CircleNumber) // 入环次数，0结束；默认2次
    {
        ...
        if (MagneticField > 220) // 直道进入，此值不宜太大，容易丢失切点
        {
            ...
            {                                 // 如果前面设置为220，此处应该大点儿，距离拉长一些，太大就走过切点了也不行！
                if (RAllPulse > TangentPointpulse + 3500)   // 进入切点后再前进3000脉冲，大约50cm，
                {
        ...
        if ((EnterCircle) && (MagneticField > 260)) // 约1.2米内再次发现强磁场，入环
        {
            LowSpeed = 500;    // 减速
            // 。左侧入环处理
...
void ElectroMagneticCar (void)
{
    ...
    while (1)
    {
        ...
        MotorDuty1 = MtTargetDuty-bat + ECPULSE1 * Kencoder - OffsetDelta * Koffset - LowSpeed;  // 有差速控制，右转偏差为负，左侧加速
